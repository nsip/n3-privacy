...
func main() {
	...
	n3cfg.SetDftCfgVal("n3-privacy", "0.0.0")
	pCfg := cfg.NewCfg(
		"Config",
		map[string]string{
			"[s]":    "Service",
...
func HostHTTPAsync(sig <-chan os.Signal, done chan<- string) {
	...
	e.GET(path, func(c echo.Context) error {
		...
		return c.String(
			http.StatusOK,
			fSf("wget   %-55s-> %s\n", fullIP+route.FetchEnfLinux, "Get Enforcer(linux64)")+
				fSf("wget   %-55s-> %s\n", fullIP+route.FetchEnfMac, "Get Enforcer(mac)")+
				fSf("wget   %-55s-> %s\n", fullIP+route.FetchEnfWin, "Get Enforcer(win64)")+
	...
	mRouteRes := map[string]string{
		route.FetchEnfLinux: Cfg.File.EnforcerLinux64,
		route.FetchEnfMac:   Cfg.File.EnforcerMac,
		route.FetchEnfWin:   Cfg.File.EnforcerWin64,
	}
